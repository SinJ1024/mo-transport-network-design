...
# Runs configs x seeds in a thread-capped parallel pool, resumable via .done markers.
#
# Usage:
#   ./jobs/gcn_ablation_desktop.sh            # run with the knobs below
#   PAR=12 NR_GROUPS=3 ./jobs/gcn_ablation_desktop.sh   # override per-run
#
...
base_cmd() {
  echo "python train_gcn.py --env=$ENV --nr_groups=$NR_GROUPS --nr_stations=$NR_STATIONS \
    --starting_loc_x=9 --starting_loc_y=19 --timesteps=$TIMESTEPS \
    --batch_size=256 --hidden_dim=$HIDDEN_DIM --lr=0.01 --max_buffer_size=100 \
    --nr_layers=1 --num_er_episodes=100 --num_model_updates=5 --num_step_episodes=10 \
    --criterion=lorenz --distance_ref=interpolate2 \
    --project_name=$PROJECT --experiment_name=GCN-${ENV}-${1} ${ENTITY:+--wandb_entity=$ENTITY}"
}
